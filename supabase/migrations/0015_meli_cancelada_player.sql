-- Flag Ops: compra cancelada por el player (badge rojo en Revisión).
-- No afecta shopper ni resultados públicos.

alter table public.meli_responses
  add column if not exists is_cancelada_player boolean not null default false;

-- Exponer isCanceladaPlayer en el listado liviano del panel
  ...
  return (
    select coalesce(jsonb_agg(
      jsonb_build_object(
        ...
        'isCanceladaPlayer', coalesce(is_cancelada_player, false),
        'nombre', nombre,
        'apellido', apellido,
...
-- Toggle desde Revisión
create or replace function public.meli_admin_set_cancelada_player(
  p_passcode text,
  p_response_id text,
  p_cancelada_player boolean
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  set
    is_cancelada_player = coalesce(p_cancelada_player, false),
    updated_at = v_now
...
grant execute on function public.meli_admin_set_cancelada_player(text, text, boolean) to anon, authenticated;

-- Detalle por token: incluir isCanceladaPlayer
...
begin
  ...
  return jsonb_build_object(
    ...
    'isCanceladaPlayer', coalesce(v_row.is_cancelada_player, false),
    'nombre', v_row.nombre,
